...
const FAIL_FOUR = ["四个字。"];
const FAIL_WORD = ["这叫单词？", "英文单词。", "查了吧。"];
const OK_LINES = ["nbb。", "接上了。", "这条算。"];
const OK_CASUAL = ["也算一轮。", "四个字就行。", "先记上。"];
...
export const lines = {
  ...
  notFour: () => pick(FAIL_FOUR),
  notWord: () => pick(FAIL_WORD),
  ok: () => pick(OK_LINES),
  okCasual: () => pick(OK_CASUAL),
  hint: () => pick(HINT_LINES),
  pass: () => pick(PASS_LINES),
  win: () => pick(WIN_LINES),
  deadEnd: () => "这个字没词了，免费过。",
  deadEndLetter: () => "这个字母没词了，免费过。",
  opening: (word: string) => `开局：${word}`,
  yourTurn: (name: string, char: string) => `${name}，接「${char}」`,
  buzzTurn: (char: string) => `抢答，接「${char}」`,
};
